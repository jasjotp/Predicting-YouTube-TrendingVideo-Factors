...
# Testing categories against number of days trending
grouped_data = [combined_data[combined_data['category_id'] == cat]['times_trending'] for cat in categories]
...
grouped_data = [exploded_data[exploded_data['tags'] == tag]['times_trending'] for tag in top_15_tags]
...
grouped_data = [combined_data[combined_data['publish_time_interval'] == interval]['times_trending'] for _, _, interval in time_bins]
...
grouped_data = [combined_data[combined_data['trending_day_of_week'] == day]['times_trending'] for day in days_of_the_week]
...
bin_labels = combined_data['description_length_bin'].unique()
grouped_data = [combined_data[combined_data['description_length_bin'] == bin_label]['times_trending'] for bin_label in bin_labels]
desc_kruskal = kruskal(*grouped_data).pvalue
print(f"Description Length vs Time Trending p-value: {desc_kruskal}")

if(desc_kruskal < 0.05):
    posthoc_data = combined_data[combined_data['description_length_bin'].isin(bin_labels)]
    posthoc = pairwise_tukeyhsd(posthoc_data['times_trending'], posthoc_data['description_length_bin'], alpha=0.05)
    print(posthoc.summary())
